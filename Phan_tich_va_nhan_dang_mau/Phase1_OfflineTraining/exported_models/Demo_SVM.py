# ==============================================================================
# PHASE 3: HỆ ĐIỀU HÀNH THỦ NGỮ (PHIÊN BẢN HIERARCHICAL SVM)
# Mô tả: Tích hợp AI Phân cấp SVM + Bộ chuẩn hóa StandardScaler.
# ==============================================================================
...
# ==========================================
# 1. NẠP HỆ THỐNG MÔ HÌNH PHÂN CẤP SVM
# ==========================================
print("[*] Đang khởi động OS Controller (Kiến trúc Phân cấp SVM)...")
MODEL_DIR = "exported_models/svm_hierarchical"

try:
    print(" -> Nạp Bộ chuẩn hóa tỷ lệ (Scaler)...")
    svm_scaler = joblib.load(f"{MODEL_DIR}/svm_scaler.pkl")

    print(" -> Nạp Mô hình Cha (Super-SVM)...")
    super_svm = joblib.load(f"{MODEL_DIR}/super_model_svm.pkl")

    print(" -> Nạp 7 Mô hình Con (Sub-SVMs)...")
    sub_models = {}
    for i in range(1, 8):
        group_id = f"G{i}"
        sub_models[group_id] = joblib.load(f"{MODEL_DIR}/sub_model_svm_{group_id}.pkl")
except Exception as e:
    print(f"[!] LỖI TẢI MÔ HÌNH: {e}")
    print("[!] Hãy đảm bảo bạn đã chạy xong file 3b_train_hierarchical_svm.py")
    exit()
...
while cap.isOpened():
    ...
    try:
        ...
        signing_hand_landmarks = None
        trigger_hand_landmarks = None
        ...
        # -------------------------------------------------------------
        # LUỒNG 1: TAY TRÁI VẬT LÝ (AI DỰ ĐOÁN CHỮ)
        # -------------------------------------------------------------
        if signing_hand_landmarks:
            ...
            # 1. CHUẨN HÓA TOÁN HỌC (Dời tâm & Thu phóng)
            normalized_row = normalize_exact_math(row)

            # Ép kiểu sang DataFrame
            feature_cols = [f'{axis}{i}' for i in range(21) for axis in ['x', 'y', 'z']]
            df_input = pd.DataFrame([normalized_row], columns=feature_cols)

            # 2. CHUẨN HÓA TỶ LỆ QUA BỘ LỌC SCALER CỦA SVM (RẤT QUAN TRỌNG)
            # Dùng transform để áp dụng đúng tỷ lệ học được từ tập Train
            df_scaled = pd.DataFrame(svm_scaler.transform(df_input), columns=feature_cols)

            # 3. AI CHA DỰ ĐOÁN CỤM
            predicted_group = super_svm.predict(df_scaled)[0]

            # 4. AI CON DỰ ĐOÁN CHỮ CÁI
            current_letter = sub_models[predicted_group].predict(df_scaled)[0]
        ...
        # -------------------------------------------------------------
        # LUỒNG 2: TAY PHẢI VẬT LÝ (NÚT XÁC NHẬN)
        # -------------------------------------------------------------
    ...
    cv2.imshow("Sign Language OS Controller (SVM Edition)", frame)
