# ==============================================================================
# PHASE 3: HỆ ĐIỀU HÀNH THỦ NGỮ (PHIÊN BẢN HIERARCHICAL KNN)
# Mô tả: Tích hợp AI Phân cấp KNN + Bộ chuẩn hóa StandardScaler.
# ==============================================================================
...
# ==========================================
# 1. NẠP HỆ THỐNG MÔ HÌNH PHÂN CẤP KNN
# ==========================================
print("[*] Đang khởi động OS Controller (Kiến trúc Phân cấp KNN)...")
MODEL_DIR = "exported_models/knn_hierarchical"

try:
    print(" -> Nạp Bộ chuẩn hóa tỷ lệ (Scaler)...")
    knn_scaler = joblib.load(f"{MODEL_DIR}/knn_scaler.pkl")

    print(" -> Nạp Mô hình Cha (Super-KNN)...")
    super_knn = joblib.load(f"{MODEL_DIR}/super_model_knn.pkl")

    print(" -> Nạp 7 Mô hình Con (Sub-KNNs)...")
    sub_models = {}
    for i in range(1, 8):
        group_id = f"G{i}"
        sub_models[group_id] = joblib.load(f"{MODEL_DIR}/sub_model_knn_{group_id}.pkl")
except Exception as e:
    print(f"[!] LỖI TẢI MÔ HÌNH: {e}")
    print("[!] Hãy đảm bảo bạn đã chạy xong file 3c_train_hierarchical_knn.py")
    exit()
...
while cap.isOpened():
    ...
    try:
        ...
        if signing_hand_landmarks:
            ...
            # 2. CHUẨN HÓA TỶ LỆ QUA BỘ LỌC SCALER CỦA KNN
            df_scaled = pd.DataFrame(knn_scaler.transform(df_input), columns=feature_cols)

            # 3. AI CHA DỰ ĐOÁN CỤM
            predicted_group = super_knn.predict(df_scaled)[0]
    ...
    cv2.imshow("Sign Language OS Controller (KNN Edition)", frame)
